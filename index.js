...
export const burger = {
  ...
  discount: function (customer) {
    if (customer == "public") { return 18 * 0.9 }
    else return 18 * 0.75
  }
}
...
*/
reviews.push({ name: "Andy", rating: 5, feedback: "Imaginary Review" })
console.log(reviews)
...
function getReviewByIndex(array, indexNumber) {
  ...
  const rating = array[indexNumber].rating
  const feedback = array[indexNumber].feedback
...
function getLastReview(reviews) {
  ...
  const name = reviews[reviews.length - 1].name
  const rating = reviews[reviews.length - 1].rating
  const feedback = reviews[reviews.length - 1].feedback
  return `${name} gave the restaurant a ${rating} star review, and their feedback was: ${feedback}`
